...
-- 9. Tabla de Sesiones de Shopify (para OAuth y API access)
-- Almacena las sesiones de acceso a la API de Shopify por tienda
...
-- Índice para búsquedas por shop
CREATE INDEX IF NOT EXISTS idx_shopify_sessions_shop ON public.shopify_sessions(shop);

-- RLS: Sin políticas = service role tiene acceso completo (las sesiones se manejan server-side)
ALTER TABLE public.shopify_sessions ENABLE ROW LEVEL SECURITY;

-- Políticas: solo service role (admin) puede acceder a sesiones
CREATE POLICY "service_role_all_shopify_sessions" ON public.shopify_sessions
    FOR ALL
    USING (true)
    WITH CHECK (true);

-- 10. Agregar columna shopify_shop a profiles (si no existe)
DO $$
BEGIN
    IF NOT EXISTS (
...
-- 12. Trigger para crear perfil automáticamente al registrarse
